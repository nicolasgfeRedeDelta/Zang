...
//variavel global criada para compartilhar o id da campanha e eu saber qual a campanha q esta sendo disparada
export class GlobalVariables {
  private static _currentCampaignId: number;

  static get currentCampaignId(): number {
    return this._currentCampaignId;
  }

  static set currentCampaignId(value: number) {
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    const whatsapp = await getWbot(schedule.whatsappId);
...
async function handleDispatchCampaign(job) {
  try {
    ...
    if (campaign.confirmation && campaignShipping.confirmation === null) {
      ...
      async function sendMessageWithDelay(message: string) {

        GlobalVariables.currentCampaignId = campaign.id;

        message = message + '\u200f'
        return new Promise<void>((resolve) => {
          setTimeout(async () => {
            await wbot.sendMessage(chatId, { text: message });
            resolve();
          }, 3000);
        });
      }
      ...
      (async () => {
        if (campaign.message1) {
          await sendMessageWithDelay(campaign.message1);
        }
        if (campaign.message2) {
          await sendMessageWithDelay(campaign.message2);
        }
        if (campaign.message3) {
          await sendMessageWithDelay(campaign.message3);
        }
        if (campaign.message4) {
          await sendMessageWithDelay(campaign.message4);
        }
        if (campaign.message5) {
          await sendMessageWithDelay(campaign.message5);
        }
        if (campaign.mediaPath) {
          const filePath = path.resolve("public", campaign.mediaPath);
          const options = await getMessageOptions("", filePath);
